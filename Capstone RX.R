...
# Note: first part of the file is creating mock data for each of the relevant datasets for analysis (end at line 361), second part is the analysis for Capstone project
...
reg1 = lm(aggregate_receivables_purchase ~ vl_inflow_net, data = ndata3)
summary(reg1)
BIC(reg1)

# account for economic condition
reg2 = lm(aggregate_receivables_purchase ~ vl_inflow_net + selic + cpi, data = ndata3)
summary(reg2)
BIC(reg2)

# account for firm performance and size in addition to economic condition
reg3 = lm(aggregate_receivables_purchase ~ vl_inflow_net + selic + cpi + nav, data = ndata3)
summary(reg3)
BIC(reg3)
anova(reg2,reg3)

#interactions (did not include in poster)
reg5 = lm(aggregate_receivables_purchase ~ vl_inflow_net + selic + cpi + nav, data = ndata3)
summary(reg5)
BIC(reg5)
anova(reg3,reg5)

reg4 = lm(aggregate_receivables_purchase ~ vl_inflow_net*nav + selic*nav + cpi*nav, data = ndata3)
summary(reg4)
BIC(reg4)
anova(reg3,reg4)
...
ggplot(ndata3, aes(x = aggregate_receivables_purchase)) +
  geom_histogram(fill = "skyblue", color = "black", bins = 30) +
  labs(x = "Aggregate Receivables Purchase", y = "Frequency") +
  theme_minimal()

ggplot(ndata3, aes(x = vl_inflow_net)) +
  geom_histogram(fill = "skyblue", color = "black", bins = 30) +
...
# Scatter plot for all predictor variables in reg3
predictor_vars <- c("vl_inflow_net", "selic", "cpi", "nav")

create_scatter_plots <- function(y_var) {
  ggplot(data = ndata3, aes_string(x = "aggregate_receivables_purchase", y = y_var)) +
    geom_point(alpha = 0.4, color = "skyblue", size = 1) +  
    geom_smooth(method = "lm", se = FALSE, color = "dark green", size = 0.6) +
    labs(x = "Aggregate Receivables Purchase", y = y_var, title = paste("Scatter Plot of", y_var)) +
    theme_minimal() +
    theme(  
...
scatter_plots <- lapply(predictor_vars, create_scatter_plots)
